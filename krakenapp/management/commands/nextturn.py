    ...
    def handle(self, *args, debug=False, **options):
        ...
        for action in actions.order_by('-type', '-date', 'creation_date'):
            ...
            if action.type == 'A':
                ...
                action.defender_id = action.target.player_id
                if action.defender_is_auto:
                    troops = min(action.defender.reserve, action.target.limit - action.target.troops)
                    if troops:
                        auto_reason = f"Renforcement automatique #{action.id} du {action.date:%x}"
                ...
                action.source.troops -= action.amount
                action.source.save(update_fields=('troops', ), _reason=reason)
                action.target.troops += action.amount
